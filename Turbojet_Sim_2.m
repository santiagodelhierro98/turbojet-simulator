function [Tt, Pt, ht, S_T, Eta_Overall, C_TS, Eta_Thermal, Eta_Propulsive] = Turbojet_Sim_2(M0, T0, P0, Tt4, TauC, epsilon_i, epsilon_b, epsilon_n, eta_cp, eta_tp, fi, x, R, hf0)
...
phi9i = phi_t9 - log(pi_9);

function F = T9_(x)
F(1) = (((3.5*log(x) - 2.8e-5*(x) + 1.12e-8*(x^2) + (3090/(x*(exp(3090/x) - 1))) - log((exp(3090/x) - 1)/exp(3090/x))) + alpha*(4.47659*log(x) + 8.01994e-3*x + 9.19648e-7*(x^2)))/(1+alpha))/(1 + alpha) - phi9i;
end

x0 = Tt9;
T9_is = fsolve(@T9_, x0);
h9air_is = h_air(T9_is);
h9fuel_is = h_fuel(T9_is);
h9_is = R*((h9air_is/R) + alpha*(h9fuel_is/R))/(1 + alpha);

Cp_n = (cp_air(Tt9) + alpha*(cp_fuel(Tt9)))/(1+alpha);
V9 = fi*sqrt(2*(ht9 - h9_is));
